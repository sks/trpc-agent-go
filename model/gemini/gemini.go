...
const (
	geminiThoughtSignatureKey = "thought_signature"
	// geminiSkipThoughtSignatureValidator is Google's documented bypass token for
	// replaying function calls that were not originally generated by the Gemini API
	// (e.g. cross-provider history from OpenAI). See:
...
func (m *Model) applyTokenTailoring(ctx context.Context, request *model.Request) {
	...
	if !modeltailoring.ApplyResult(ctx, "gemini.Model", request, tailored) {
		return
	}
