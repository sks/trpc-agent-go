...
func (s *sessionSummarizer) selectSummaryEvents(
	...
	if !ok {
		events := filterSummaryInputEventsForSession(
			s.filterEventsForSummary(sess.GetVisibleEvents()),
			sess,
		)
	...
	if boundary, found := view.BoundaryForItems(itemIndexes); found {
		...
		if source := sourceEventsThroughBoundary(sess.GetVisibleEvents(), boundary); len(source) > 0 {
...
func (s *sessionSummarizer) Summarize(ctx context.Context, sess *session.Session) (string, error) {
	...
	if len(sess.GetVisibleEvents()) == 0 && (!separatePreviousSummary || previousSummary == "") {
